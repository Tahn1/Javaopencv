...
public class GradeDetailFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        // 1) Thu nhỏ font title khi fragment hiện
        AppCompatActivity act = (AppCompatActivity) requireActivity();
        Toolbar hostToolbar = act.findViewById(R.id.toolbar); // <-- thay bằng id thật của Toolbar trong Activity
        if (hostToolbar != null) {
            hostToolbar.setTitleTextAppearance(requireContext(),
        ...
        vm.getGradeResult().observe(getViewLifecycleOwner(), gr -> {
            if (gr == null) return;
            // Set title lên ActionBar chung (font đã nhỏ)
